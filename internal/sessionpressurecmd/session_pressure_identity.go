...
const sessionPressureIdentityHelp = `Usage: ndev [--json] session pressure identity show [--live]
...
func cmdSessionPressureIdentity(g *Flags, args []string) int {
	if len(args) == 0 {
		return sessionPressureError("identity requires a subcommand; try: ndev session pressure identity show", 2)
	}
	switch args[0] {
	case "help", "--help", "-h":
