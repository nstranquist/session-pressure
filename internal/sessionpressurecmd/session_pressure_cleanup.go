...
const sessionPressureCleanupHelp = `Usage: ndev [--json] session pressure cleanup <subcommand>
...
Examples:
  ndev --json session pressure cleanup plan
  ndev session pressure cleanup policy enable
  ndev session pressure cleanup claim acquire --kind dev_session --resource ad-hoc/api --owner terminal-1 --ttl 2h
  ndev session pressure cleanup claim heartbeat --claim-id claim-...
`
