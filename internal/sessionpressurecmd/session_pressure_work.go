...
var sessionPressureWorkHelp = fmt.Sprintf(`Usage: ndev [--json] session pressure work <subcommand>
...
func cmdSessionPressureWork(g *Flags, args []string) int {
	...
	default:
		return sessionPressureError("unknown work subcommand "+strconv.Quote(sub)+"; try: ndev session pressure work --help (status|run|batch|override|history|stats|report|evaluate)", 2)
	}
}
...
func cmdSessionPressureWorkReport(g *Flags, dir string, args []string) int {
	...
	if calibration.SuggestedPolicyProfile != "" {
		text += fmt.Sprintf("  suggested_profile=%s reason=%s  # advisory only; try: ndev session pressure policy profile apply %s --dry-run\n",
