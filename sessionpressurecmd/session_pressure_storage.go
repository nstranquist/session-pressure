...
func cmdSessionPressureStorage(g *Flags, args []string) int {
	...
	switch sub {
	case "help", "--help", "-h":
		fmt.Print(`Usage: session-pressure [--json] storage <subcommand> [flags]
...
func inspectStorageProviders(policy sessionpressure.Policy, measure bool) ([]storageProviderReport, error) {
	...
	profilesReport := storageProviderReport{ID: "browser-dead-profiles", Classification: storageProviderAutoSafe, Summary: "Dead PageSkein profiles older than the lifecycle floor", MutationSupported: false, Present: true, EstimateKind: "exact_gc", BlockedReason: browser.ErrNotAvailable.Error()}
	if report, err := storageBrowserGC(browser.GCOptions{OlderThan: time.Duration(policy.Storage.ProviderMinAgeSeconds) * time.Second}); err == nil && report != nil {
		profilesReport.EstimatedBytes = report.ReclaimableBytes
		profilesReport.BlockedReason = ""
		profilesReport.MutationSupported = true
	} else if err != nil {
...
func executeStorageProvider(ctx context.Context, runtime pressureRuntime, provider storageProviderReport) ([]sessionpressure.StorageReceipt, error) {
	...
	case "browser-dead-profiles":
		report, gcErr := storageBrowserGC(browser.GCOptions{OlderThan: time.Duration(runtime.policy.Storage.ProviderMinAgeSeconds) * time.Second, Apply: true})
		if gcErr != nil {
			runErr = gcErr
		} else if report == nil {
			runErr = browser.ErrNotAvailable
		}
	case "user-trash":
