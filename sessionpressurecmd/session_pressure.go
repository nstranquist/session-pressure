...
const sessionPressureHelp = `Usage: session-pressure [--json] <subcommand> [flags]
...
Examples:
  session-pressure --json doctor
  session-pressure --json audit --since 24h
  session-pressure --json snapshot
  session-pressure --json self-test --wait 10m
  session-pressure monitor install
  session-pressure policy enable
  session-pressure policy profile apply observe
  session-pressure --json work report --since 24h
  session-pressure work run --wait 0 -- go test ./internal/sessionpressure
  session-pressure work run --class express-test -- go test ./internal/sessionpressure
  session-pressure work run --class test -- go test ./...
  session-pressure work run --class benchmark-exclusive -- go test -bench . ./...
	  session-pressure --json storage plan --target-free 50GiB
	  session-pressure --json io status --live
  session-pressure --json telemetry --since 24h --limit 20
  session-pressure --json idle --min-age 12h --limit 10
  session-pressure idle --apply --root-pid PID --session-id ID
`
...
func cmdSessionPressure(g *Flags, args []string) int {
	args = StripSessionPressurePrefix(args)
	sub := "snapshot"
	if len(args) > 0 {
		sub = args[0]
		args = args[1:]
	...
	default:
		...
		return sessionPressureError("unknown subcommand "+strconv.Quote(sub)+"; try: session-pressure --help (status|doctor|work|policy|monitor|snapshot|check|audit|recovery|telemetry|idle|cleanup|artifact|storage|io|api|identity)", 2)
	}
}
...
func sessionPressureError(message string, exit int) int {
	...
	fmt.Fprintln(os.Stderr, "session-pressure:", message)
	return exit
}
