...
func cmdSessionPressureArtifact(g *Flags, args []string) int {
	...
	case "status", "plan":
		...
		if drift.Drifted {
			text += "artifact DRIFT: a newer build is not promoted; coordinated work still runs the installed revision.\n" +
				"  promote with: session-pressure monitor install\n"
		}
	...
	case "prune":
		...
		if err := flags.Parse(args); err != nil || flags.NArg() != 0 {
			return sessionPressureError("usage: session-pressure artifact prune [--apply]", 2)
		}
	...
	case "help", "--help", "-h":
		fmt.Print("Usage: session-pressure [--json] artifact status|plan|prune [--apply]\n")
		return 0
	default:
